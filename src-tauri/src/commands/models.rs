...
pub fn switch_active_model(app: &AppHandle, model_id: &str) -> Result<(), String> {
    ...
    // Dicteren.ai is Dutch-only; reset stale language settings from old installs.
    if settings.selected_language != "nl"
    ...
    {
        log::info!(
            "Resetting language from '{}' to 'nl' (Dicteren.ai is Dutch-only; model {})",
            settings.selected_language,
            model_id
        );
        settings.selected_language = "nl".to_string();
    }
