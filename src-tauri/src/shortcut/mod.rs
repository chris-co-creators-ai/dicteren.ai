...
//! - `dicteren_keys`: Uses the dicteren-keys library for more control
//!
//! The active implementation is determined by the `keyboard_implementation`
//! setting and can be changed at runtime.

pub mod dicteren_keys;
mod handler;
mod tauri_impl;
...
// Note: Commands are accessed via shortcut::dicteren_keys:: in lib.rs
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::DicterenKeys => {
            if let Err(e) = dicteren_keys::init_shortcuts(app) {
                error!("Failed to initialize dicteren-keys shortcuts: {}", e);
                ...
                // Update settings to persist the fallback so we don't retry DicterenKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::DicterenKeys => dicteren_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::DicterenKeys => dicteren_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::DicterenKeys => dicteren_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::DicterenKeys => dicteren_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn change_keyboard_implementation_setting(
    ...
    // Initialize new implementation if needed (DicterenKeys needs state)
    if new_impl == KeyboardImplementation::DicterenKeys {
        if initialize_dicteren_keys_with_rollback(&app)? {
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::DicterenKeys => "dicteren_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::DicterenKeys => dicteren_keys::validate_shortcut(raw),
    }
}

/// Parse a keyboard implementation string into the enum
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    let upstream_legacy_value = ["han", "dy_keys"].concat();
    match s {
        "tauri" => KeyboardImplementation::Tauri,
        "dicteren_keys" => KeyboardImplementation::DicterenKeys,
        legacy if legacy == upstream_legacy_value => KeyboardImplementation::DicterenKeys,
        other => {
            warn!(
...
fn unregister_all_shortcuts(app: &AppHandle, implementation: KeyboardImplementation) {
    ...
    for (id, binding) in bindings {
        ...
        let result = match implementation {
            KeyboardImplementation::Tauri => tauri_impl::unregister_shortcut(app, binding),
            KeyboardImplementation::DicterenKeys => {
                dicteren_keys::unregister_shortcut(app, binding)
            }
        };
...
/// Initialize DicterenKeys if not already initialized, with rollback on failure
fn initialize_dicteren_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app
        .try_state::<dicteren_keys::DicterenKeysState>()
        .is_some()
    {
    ...
    if let Err(e) = dicteren_keys::init_shortcuts(app) {
        error!("Failed to initialize DicterenKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize DicterenKeys: {}. Reverted to Tauri.",
            e
        ));
    }
