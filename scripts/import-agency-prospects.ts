// ScrapeGraphAI-output → CRM. Verdeelt marketingbureau-prospects round-robin over
// alle account managers, elk in een eigen (gedeelde) lead-lijst, en importeert via
// de bestaande importEnrichedProspects-service. Idempotent op (org, email).
//
// Run: cd web && bun --conditions=react-server scripts/import-agency-prospects.ts [pad/prospects.json]
//
// prospects.json = array van EnrichedProspectRow (email verplicht). De ScrapeGraphAI-
// scraper (.claude/skills/scrapegraph-official/agency_scraper.py) schrijft dit bestand.
...
const SOURCE = "scrapegraph-maps";
const COLORS = ["blue", "green", "orange", "purple", "aqua", "navy", "red", "gray"] as const;
const jsonPath = process.argv[2] ?? "scripts/data/prospects.json";

const sql = neon(process.env.DATABASE_URL!);

// 1. Account managers ophalen — SSOT is de rol in de DB, geen hardcoded ids.
const ams = (await sql`
...
async function ensureList(
  ...
  const row = await createLeadList({
    ...
    description: "Round-robin marketingbureaus, gescrapet via ScrapeGraphAI (Google Maps).",
  });
  return row.id;
}
