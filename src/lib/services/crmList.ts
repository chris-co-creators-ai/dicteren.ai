...
export type CrmPeopleFilters = {
  ...
  /** Funnel-spoor: eindklant-werving vs reseller-werving (migratie 0052). */
  prospectType?: "eindklant" | "reseller" | null;
};
...
function buildConditions(filters: CrmPeopleFilters): SQL[] {
  ...
  if (filters.prospectType) {
    conds.push(sql`people.prospect_type = ${filters.prospectType}`);
  }
