...
- **Funnel-split (migratie 0052):** `crm_contacts.prospect_type` (`eindklant`|`reseller`) is de discriminator die eindklant- van reseller-werving scheidt. Het Kanban heeft een Eindklant|Reseller-toggle: het eindklant-bord toont de sleepbare customer-stages; het reseller-bord toont kolommen afgeleid van de deck-timestamps (`deck_sent_at`/`deck_visited_at`/`commission_discussed_at`/`applied_at`/`promoted_affiliate_id`) en is **read-only** (voortgang via de Partner-tab, niet via slepen — geen aparte reseller-stage-enum). De **Partner-tab** in het personen-side-panel toont voorlopig voor **alle** prospects (bewust: 'm verbergen op `prospect_type` zou de halve partner-pipeline verstoppen zolang de 230 leads nog niet handmatig geclassificeerd zijn). De reseller-only-beperking komt pas terug nadat de classificatie compleet is. `lead_lists.list_type` zet de default voor nieuwe imports. Classificeren: bulk "Set funnel" (`/api/admin/crm/contacts/prospect-type`) of "Hele lijst → funnel" (`/api/admin/lead-lists/[id]/prospect-type`), services in `crmAssign.ts`. De afleiding leeft in de `PEOPLE_CTE` van `crmList.ts` (`reseller_stage`). Raakt de bestaande reseller-flow (deck/mails/promote) niet.
