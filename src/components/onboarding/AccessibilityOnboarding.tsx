...
const AccessibilityOnboarding: React.FC<AccessibilityOnboardingProps> = ({
  ...
  const checkMacPermissionsWithFallback = useCallback(async () => {
    const [accessibilityGrantedRaw, microphoneGranted] = await Promise.all([
      checkAccessibilityPermission(),
      checkMicrophonePermission(),
    ]);

    // On some macOS setups the accessibility check can lag after toggling.
    // If Enigo initializes successfully, treat accessibility as granted.
    let accessibilityGranted = accessibilityGrantedRaw;
    if (!accessibilityGrantedRaw) {
      try {
        const enigoInit = await commands.initializeEnigo();
        if (enigoInit.status === "ok") {
  ...
  useEffect(() => {
      ...
      if (nextPlatform === "macos") {
        try {
          const { accessibilityGranted, microphoneGranted } =
            await checkMacPermissionsWithFallback();
    ...
    checkMacPermissionsWithFallback,
    completeOnboarding,
  ...
  const handleManualRecheck = useCallback(async () => {
    try {
      ...
      console.error("Manual permission recheck failed:", error);
      toast.error(t("onboarding.permissions.errors.checkFailed"));
    }
  }, [
    checkMacPermissionsWithFallback,
  ...
  if (isChecking) {
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-white">
        <Loader2 className="w-8 h-8 animate-spin text-[#0A2A73]/50" />
      </div>
    );
  }
  ...
  if (allGranted) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center gap-4 bg-white">
        ...
        <p className="text-lg font-medium text-[#0A2A73]">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {permissions.microphone === "granted" ? (
                  ...
                  <div className="flex items-center gap-2 text-[#0A2A73]/60 text-sm">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {permissions.accessibility === "granted" ? (
                  ...
                  <div className="flex items-center gap-3 text-[#0A2A73]/60 text-sm">
                    ...
                    <button
                      ...
                      className="px-3 py-1 rounded-md bg-white border border-[#d6e5fa] text-[#0A2A73] hover:bg-[#f3f8ff] transition-colors"
                    >
                      Opnieuw controleren
                    </button>
                  </div>
