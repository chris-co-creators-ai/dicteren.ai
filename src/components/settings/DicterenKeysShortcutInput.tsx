...
interface DicterenKeysShortcutInputProps {
...
export const DicterenKeysShortcutInput: React.FC<DicterenKeysShortcutInputProps> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await commands.stopDicterenKeysRecording().catch(console.error);
  ...
  // Set up event listener for dicteren-keys events
  useEffect(() => {
    if (!isRecording) return;
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<DicterenKeysEvent>(
        "dicteren-keys-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      commands.stopDicterenKeysRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      await commands.startDicterenKeysRecording(shortcutId);
      setIsRecording(true);
      setCurrentKeys("");
      currentKeysRef.current = "";
    } catch (error) {
        ...
        {isRecording ? (
          <div
            ...
            style={{
              ...
              boxShadow: "0 0 0 3px rgba(255, 132, 65, 0.18)",
              padding: "0 10px",
              height: 26,
              minWidth: 26,
              fontSize: 12,
